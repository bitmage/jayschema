...
var JaySchema = require('./jaySchema.js')
  , http = require('http')
  , https = require('https')
  , url = require('url')
  ;
...
function loader(ref, callback, _depth) {
  ...
  if (_depth >= MAX_DEPTH) {
    ...
    var err = new JaySchema.Errors.SchemaLoaderError(ref, desc);
    return callback(err);
  }
  ...
  getter(ref, function(res) {
    ...
    res.on('end', function() {
      try {
        ...
        var err = new JaySchema.Errors.SchemaLoaderError(ref, desc, jsonError);
        return callback(err);
      }
    });
    ...
    res.on('error', function(httpError) {
      ...
      var err = new JaySchema.Errors.SchemaLoaderError(ref, desc, httpError);
      callback(err);
    });
